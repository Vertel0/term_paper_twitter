#let template(cfg: none, body) = {
    ...
    let un(n) = "_" * n
    let approval_page_code = cfg.project.code + "-ПЗ"
    ...
    let approval_page = {
        ...
        let approve_table = grid(
            ...
            [СОГЛАСОВАНО\ #cfg.agreed_by.position],
            [УТВЕРЖДЕНО\ #cfg.approved_by.position],
        ...
        let bottom_banner = [#set text(weight: "bold"); #cfg.year]
        ...
        page(
            ...
            #set align(center)
            #grid(columns: (1fr), row-gutter: 1fr, top_banner, approve_table, center_banner, student_info, bottom_banner)
        ]
    ...
    let title_page = {
        ...
        page(
            ...
            #set align(center)
            #grid(columns: (1fr), row-gutter: 1fr, top_banner, center_banner, bottom_banner)
        ]
    }
    ...
    let list_registraion_changes = {
        ...
        page(
            ...
            #set align(center)
            #grid(columns: (1fr), row-gutter: 1fr, list_name, changes_table)
        ]
    }
    ...
    let outline_and_normal_pages = {
        ...
        show heading.where(level: 1): h => {
            ...
            pagebreak(weak: true)
            if h.numbering != none [#counter(heading).display(h.numbering) #h.body] else [#h.body]
        }
